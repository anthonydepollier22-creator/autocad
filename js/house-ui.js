...
function initHouseUI(app) {
  ...
  function ensureDesign(force) {
    ...
    editor.routeLines = design && design.ok && typeof routeSegments === 'function' ? routeSegments(design, editor.components) : null;
  ...
  function action(act) {
    ...
    if (act === 'routes') {
      editor.showRoutes = !editor.showRoutes;
      try { localStorage.setItem('electricad-routes', editor.showRoutes ? '1' : '0'); } catch (_) { /* stockage indisponible */ }
      ensureDesign(); editor.render(); structKey = null; tick(0, true);
      if (editor.showRoutes) showToast('Câbles sur le plan : chaque circuit suit les goulottes jusqu’à ses appareils, à la couleur de son repère.', 4000);
      return;
    }
    if (act === 'tags') {
