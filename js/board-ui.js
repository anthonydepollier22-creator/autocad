...
function initBoardUI(app) {
  ...
  function render() {
    ...
    for (const g of groups) {
      ...
      for (const c of g.cs) {
        ...
        const len = c.devices.length || c.link ? `<span class="bd-ro" title="${c.link ? 'Ligne mesurée sur le plan, dans les goulottes, du tableau principal au tableau divisionnaire' : 'Mesurée sur le plan'}">${num(c.length, 1)} m</span>` : `<input type="number" min="1" max="200" step="0.5" data-cf="length" value="${num(c.length, 1).replace(/\s/g, '').replace(',', '.')}" aria-label="Longueur"><em>m</em>`;
        h += `<tr data-c="${esc(c.id)}" class="${fc ? fc.cls : ''}"${fc ? ` title="${esc(fc.tip)}"` : ''}>` +
          ...
          `<td class="bd-acts">` + (c.kind === 'sub' || c.kind === 'pv' || /ext[ée]rieur|jardin|portail|abri|piscine|garage/i.test(c.name) ? `<button type="button" data-cact="bur" aria-pressed="${c.buried ? 'true' : 'false'}" title="Liaison enterrée : câble U1000 R2V sous fourreau TPC rouge (métré, unifilaire)">⛏</button>` : '') +
  ...
  function renderPreview(d) {
    ...
    const avail = { elev: hasPlan(), vdi: editor.components.some((c) => c.type === 'rj45'), heat: !!(d && d.ok && d.circuits.some((c) => c.kind === 'heating')) };
    ...
    } else if (st.view === 'calc' || st.view === 'dev' || st.view === 'vdi' || st.view === 'elev' || st.view === 'wiring' || st.view === 'labels' || st.view === 'nomen' || st.view === 'heat') {
      const pages = st.view === 'calc' ? calcNoteSVGs(d, meta()) : st.view === 'dev' ? devSVGs(d) : st.view === 'elev' ? elevPages(d) : st.view === 'wiring' ? boardWiringSVGs(d, meta()) : st.view === 'labels' ? boardLabelsSVGs(d, meta()) : st.view === 'nomen' ? nomenPages(d) : st.view === 'heat' ? heatPages(d) : vdiPages(d);
      st.folio = Math.min(st.folio, pages.length - 1);
      svg = pages[st.folio];
      $('bd-folios').hidden = pages.length < 2;
      $('bd-folio-lbl').textContent = `Folio ${st.folio + 1} / ${pages.length}`;
  ...
  modal.querySelectorAll('[data-bx]').forEach((b) => b.addEventListener('click', () => {
    ...
    if (k === 'svg') {
      ...
      const svg = st.view === 'uni' ? unifilarSVG(d, meta()) : st.view === 'calc' ? folio(calcNoteSVGs(d, meta())) : st.view === 'dev' ? folio(devSVGs(d)) : st.view === 'vdi' ? folio(vdiPages(d)) : st.view === 'elev' ? folio(elevPages(d)) : st.view === 'wiring' ? folio(boardWiringSVGs(d, meta())) : st.view === 'front' ? boardFrontSVG(d, meta()) : st.view === 'nomen' ? folio(nomenPages(d)) : st.view === 'heat' ? folio(heatPages(d)) : folio(boardLabelsSVGs(d, meta()));
      const what = { uni: 'unifilaire', calc: 'note de calcul', dev: 'schémas développés', vdi: 'communication', elev: 'élévations', wiring: 'câblage du tableau', front: 'face avant', labels: 'étiquettes', nomen: 'nomenclature', heat: 'chauffage fil pilote' }[st.view];
      ...
      } else if (st.view === 'heat') {
        download(new Blob([heatingDXF(d, meta(), editor.components)], { type: 'application/dxf' }), fileName(base() + ' - chauffage fil pilote.dxf'));
        showToast('Raccordement du chauffage exporté en <b>DXF</b> (millimètres, calques SCHEMA, TEXTES, CARTOUCHE).', 3500);
