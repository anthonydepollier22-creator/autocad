...
function selfCheckList(report, design) {
  ...
  if (design && design.ok) {
    ...
    add('Tous les circuits sous interrupteur différentiel 30 mA', design.circuits.every((c) => c.rcd || c.ddr || c.kind === 'sub') ? 'ok' : 'err'); // départ de TD : ID dans le TD
...
function buildDossier(o) {
  ...
  if (d && d.ok) {
    ...
    if (o.wiringSVGs && o.wiringSVGs.length) h += o.wiringSVGs.map((svg) => `<section class="page"><h2>Câblage du tableau</h2><div class="svg">${svg}</div></section>`).join('');
    ...
    if (o.heatingSVGs && o.heatingSVGs.length) h += o.heatingSVGs.map((svg, k) => `<section class="page"><h2>Chauffage — raccordement fil pilote${o.heatingSVGs.length > 1 ? ` (${k + 1}/${o.heatingSVGs.length})` : ''}</h2><div class="svg">${svg}</div></section>`).join('');
    if (o.shutterSVGs && o.shutterSVGs.length) h += o.shutterSVGs.map((svg, k) => `<section class="page"><h2>Volets roulants${o.shutterSVGs.length > 1 ? ` (${k + 1}/${o.shutterSVGs.length})` : ''}</h2><div class="svg">${svg}</div></section>`).join('');
